...
interface JournalEntryModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave?: (entry: any, status: 'draft' | 'validated') => void;
  initialData?: any;
  mode?: 'create' | 'edit';
}
...
const JournalEntryModal: React.FC<JournalEntryModalProps> = ({
  ...
  // Construire l'objet écriture à partir de l'état du formulaire
  const buildEntryData = useCallback((status: 'draft' | 'validated') => {
    return {
      id: initialData?.id || crypto.randomUUID(),
      entryNumber: details.numeroEcriture,
      journal: details.journal.split(' - ')[0],
      date: details.dateEcriture,
      reference: details.reference,
      label: details.description,
      status,
      lines: lignesEcriture.map((ligne, idx) => ({
        id: `${initialData?.id || 'new'}-line-${idx}`,
        accountCode: ligne.compte,
        accountName: ligne.libelle,
        label: ligne.libelle,
        debit: ligne.debit,
        credit: ligne.credit,
        analyticalCode: ligne.codeAnalytique || undefined,
      })),
      totalDebit,
      totalCredit,
      notes: notes.commentairesGeneraux || undefined,
      createdAt: initialData?.createdAt || new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    };
  }, [initialData, details, lignesEcriture, totalDebit, totalCredit, notes]);

  // Sauvegarder en brouillon
  const handleSaveDraft = useCallback(() => {
    const entry = buildEntryData('draft');
    if (onSave) {
      onSave(entry, 'draft');
    }
    handleClose();
  }, [buildEntryData, onSave, handleClose]);

  // Valider et comptabiliser
  const handleValidateAndPost = useCallback(() => {
    if (validationErrors.length > 0) return;
    const entry = buildEntryData('validated');
    if (onSave) {
      onSave(entry, 'validated');
    }
    handleClose();
  }, [buildEntryData, onSave, handleClose, validationErrors]);
  ...
  }, [activeTab, goToNextTab, goToPrevTab, validationErrors, handleValidateAndPost]);
