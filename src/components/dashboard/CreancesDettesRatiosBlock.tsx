...
const DSO_MIN_THRESHOLD_DEFAULT = 15; const DSO_MIN_THRESHOLD_KEY = 'manager-dso-min-threshold'; // j — alerte si DSO en dessous (crédit clients trop strict)
...
const CreancesDettesRatiosBlock: React.FC<Props> = ({ entries, period, showAlerts = true }) => {
  ...
  const [dsoMinThreshold, setDsoMinThreshold] = useState<number>(() => readNum(DSO_MIN_THRESHOLD_KEY, DSO_MIN_THRESHOLD_DEFAULT));
  ...
  if (data.dso > 0 && data.dso < dsoMinThreshold) alerts.push(`DSO trop court : ${data.dso} j (min ${dsoMinThreshold} j) — conditions de crédit clients peut-être trop strictes`);
  ...
  const cards = [
    ...
    { label: 'DSO', value: `${data.dso} j`, sub: `Recouvrement · cible ${dsoMinThreshold}–${dsoThreshold} j`, icon: Clock, tone: (data.dso > dsoThreshold || (data.dso > 0 && data.dso < dsoMinThreshold)) ? 'text-[var(--color-error)]' : 'text-[var(--color-text-primary)]' },
    ...
    <section className="bg-white rounded-xl p-6 shadow-sm border border-[var(--color-border)] mb-8">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <label className="flex items-center gap-1.5 text-xs text-[var(--color-text-secondary)]">
            DSO min
            <input type="number" min={0} max={365} value={dsoMinThreshold} onChange={(e) => persist(DSO_MIN_THRESHOLD_KEY, setDsoMinThreshold, Number(e.target.value), 0, 365)} className={`w-14 ${inputCls}`} aria-label="Seuil DSO minimum en jours" /> j
          </label>
          <label className="flex items-center gap-1.5 text-xs text-[var(--color-text-secondary)]">
            Seuil DPO
