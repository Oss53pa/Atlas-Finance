  ...
  if (!currentData) {
    return (
      <div className="text-center p-8">
        <p className="text-[var(--color-primary)]/50">{t('cashflowStmt.noData')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-lg font-bold text-[var(--color-primary)] flex items-center">
              ...
              {t('cashflowStmt.title')}
            </h1>
            <p className="text-sm text-[var(--color-primary)]/70 mt-1">{t('cashflowStmt.fiscalYearSubtitle', { year: fiscalYear })}</p>
          </div>
          <div className="flex items-center space-x-3">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm font-medium">{d.isCashFlowBalanced ? t('cashflowStmt.balanced') : t('cashflowStmt.unbalanced')}</span>
            </div>
            <button className="flex items-center space-x-2 px-3 py-2 border border-[var(--color-border)] rounded-md hover:bg-[var(--color-border)]">
              <ArrowDownTrayIcon className="h-4 w-4" /><span>{t('cashflowStmt.export')}</span>
            </button>
            <button className="flex items-center space-x-2 px-3 py-2 bg-[var(--color-text-secondary)] text-white rounded-md hover:bg-[#404040]">
              <DocumentTextIcon className="h-4 w-4" /><span>{t('cashflowStmt.pdf')}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { label: t('cashflowStmt.kpiOperating'), value: d.operatingCashFlow, sub: activeTab === 'indirect' && indirectData ? t('cashflowStmt.kpiSelfFinancing', { value: fmt(indirectData.selfFinancingCapacity) }) : t('cashflowStmt.kpiOperatingSub') },
          { label: t('cashflowStmt.kpiInvesting'), value: d.investmentCashFlow, sub: t('cashflowStmt.kpiInvestingSub') },
          { label: t('cashflowStmt.kpiFinancing'), value: d.financingCashFlow, sub: t('cashflowStmt.kpiFinancingSub') },
          { label: t('cashflowStmt.kpiVariation'), value: d.cashFlowVariation, sub: t('cashflowStmt.kpiVariationSub') },
        ].map((kpi, i) => (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead className="bg-[var(--color-primary)]">
                <tr>
                  <th className="px-6 py-4 text-left text-sm font-bold text-white">{t('cashflowStmt.tableHeaderIndirect')}</th>
                  <th className="px-6 py-4 text-right text-sm font-bold text-white w-48">{t('cashflowStmt.amountColumn')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[var(--color-border)]">
                <SectionHeader title={t('cashflowStmt.sectionA')} />
                <Row label={t('cashflowStmt.netResult')} value={indirectData.netResult} indent />
                <Row label={t('cashflowStmt.depreciationProvisions')} value={indirectData.depreciationAndProvisions} indent sign="+" />
                <Row label={t('cashflowStmt.provisionsReversals')} value={indirectData.provisionsReversals} indent sign="-" />
                <Row label={t('cashflowStmt.valueAdjustments')} value={indirectData.valueAdjustments} indent />
                <Row label={t('cashflowStmt.selfFinancingCapacity')} value={indirectData.selfFinancingCapacity} bold bg="bg-[var(--color-primary)]/10" />
                <Row label={t('cashflowStmt.workingCapitalVariation')} value={indirectData.workingCapitalVariation} indent sign="-" />
                <Row label={t('cashflowStmt.operatingNetFlow')} value={indirectData.operatingCashFlow} bold bg="bg-[var(--color-primary)]/20" />

                <SectionHeader title={t('cashflowStmt.sectionB')} />
                <Row label={t('cashflowStmt.fixedAssetsAcquisitions')} value={indirectData.fixedAssetsAcquisitions} indent sign="-" />
                <Row label={t('cashflowStmt.fixedAssetsDisposals')} value={indirectData.fixedAssetsDisposals} indent sign="+" />
                <Row label={t('cashflowStmt.financialAssetsAcquisitions')} value={indirectData.financialAssetsAcquisitions} indent sign="-" />
                <Row label={t('cashflowStmt.financialAssetsDisposals')} value={indirectData.financialAssetsDisposals} indent sign="+" />
                <Row label={t('cashflowStmt.investingNetFlow')} value={indirectData.investmentCashFlow} bold bg="bg-[var(--color-primary)]/20" />

                <SectionHeader title={t('cashflowStmt.sectionC')} bg="bg-[var(--color-text-secondary)]/10" />
                <Row label={t('cashflowStmt.capitalIncrease')} value={indirectData.capitalIncrease} indent sign="+" />
                <Row label={t('cashflowStmt.investmentSubsidies')} value={indirectData.investmentSubsidiesReceived} indent sign="+" />
                <Row label={t('cashflowStmt.newBorrowings')} value={indirectData.newBorrowings} indent sign="+" />
                <Row label={t('cashflowStmt.loanRepayments')} value={indirectData.loanRepayments} indent sign="-" />
                <Row label={t('cashflowStmt.dividendsPaid')} value={indirectData.dividendsPaid} indent sign="-" />
                <Row label={t('cashflowStmt.financingNetFlow')} value={indirectData.financingCashFlow} bold bg="bg-[var(--color-text-secondary)]/20" />

                <SectionHeader title={t('cashflowStmt.sectionD')} bg="bg-[var(--color-text-secondary)]/10" />
                <Row label={t('cashflowStmt.netCashVariation')} value={indirectData.cashFlowVariation} bold bg="bg-[var(--color-text-secondary)]/20" />
                <Row label={t('cashflowStmt.openingCash')} value={indirectData.openingCashBalance} indent />
                <Row label={t('cashflowStmt.closingCash')} value={indirectData.closingCashBalance} indent />
                <tr className={indirectData.isCashFlowBalanced ? 'bg-green-50' : 'bg-red-50'}>
                  <td className="px-6 py-3 font-bold flex items-center">
                    ...
                    {t('cashflowStmt.controlVariance')}
                  </td>
        ...
        <div className="bg-[var(--color-surface-hover)] rounded-lg shadow-sm border border-[var(--color-border)] overflow-hidden">
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead className="bg-[var(--color-primary)]">
                <tr>
                  <th className="px-6 py-4 text-left text-sm font-bold text-white">{t('cashflowStmt.tableHeaderDirect')}</th>
                  <th className="px-6 py-4 text-right text-sm font-bold text-white w-48">{t('cashflowStmt.amountColumn')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[var(--color-border)]">
                <SectionHeader title={t('cashflowStmt.sectionA')} />
                <Row label={t('cashflowStmt.receiptsFromCustomers')} value={directData.encaissementsClients} indent sign="+" />
                <Row label={t('cashflowStmt.otherOperatingReceipts')} value={directData.autresEncaissementsExploit} indent sign="+" />
                <Row label={t('cashflowStmt.paymentsToSuppliers')} value={directData.decaissementsFournisseurs} indent sign="-" />
                <Row label={t('cashflowStmt.paymentsToStaff')} value={directData.decaissementsPersonnel} indent sign="-" />
                <Row label={t('cashflowStmt.interestPaid')} value={directData.interetsPayes} indent sign="-" />
                <Row label={t('cashflowStmt.incomeTaxPaid')} value={directData.impotsSurResultat} indent sign="-" />
                <Row label={t('cashflowStmt.otherOperatingPayments')} value={directData.autresDecaissementsExploit} indent sign="-" />
                <Row label={t('cashflowStmt.operatingNetFlow')} value={directData.operatingCashFlow} bold bg="bg-[var(--color-primary)]/20" />

                <SectionHeader title={t('cashflowStmt.sectionB')} />
                <Row label={t('cashflowStmt.paymentsTangibleAcq')} value={directData.acquisitionsImmosCorporelles} indent sign="-" />
                <Row label={t('cashflowStmt.paymentsIntangibleAcq')} value={directData.acquisitionsImmosIncorporelles} indent sign="-" />
                <Row label={t('cashflowStmt.paymentsFinancialAcq')} value={directData.acquisitionsImmosFinancieres} indent sign="-" />
                <Row label={t('cashflowStmt.receiptsTangibleDisp')} value={directData.cessionsImmosCorporelles} indent sign="+" />
                <Row label={t('cashflowStmt.receiptsFinancialDisp')} value={directData.cessionsImmosFinancieres} indent sign="+" />
                <Row label={t('cashflowStmt.interestDividendsReceived')} value={directData.interetsDividendesRecus} indent sign="+" />
                <Row label={t('cashflowStmt.investingNetFlow')} value={directData.investmentCashFlow} bold bg="bg-[var(--color-primary)]/20" />

                <SectionHeader title={t('cashflowStmt.sectionC')} bg="bg-[var(--color-text-secondary)]/10" />
                <Row label={t('cashflowStmt.receiptsCapitalIncrease')} value={directData.augmentationCapital} indent sign="+" />
                <Row label={t('cashflowStmt.investmentSubsidies')} value={directData.subventionsRecues} indent sign="+" />
                <Row label={t('cashflowStmt.receiptsFromBorrowings')} value={directData.empruntsNouveaux} indent sign="+" />
                <Row label={t('cashflowStmt.loanRepaymentsDirect')} value={directData.remboursementsEmprunts} indent sign="-" />
                <Row label={t('cashflowStmt.dividendsPaidDirect')} value={directData.dividendesVerses} indent sign="-" />
                <Row label={t('cashflowStmt.financingNetFlow')} value={directData.financingCashFlow} bold bg="bg-[var(--color-text-secondary)]/20" />

                <SectionHeader title={t('cashflowStmt.sectionD')} bg="bg-[var(--color-text-secondary)]/10" />
                <Row label={t('cashflowStmt.netCashVariation')} value={directData.cashFlowVariation} bold bg="bg-[var(--color-text-secondary)]/20" />
                <Row label={t('cashflowStmt.openingCash')} value={directData.openingCashBalance} indent />
                <Row label={t('cashflowStmt.closingCash')} value={directData.closingCashBalance} indent />
      ...
      <div className="bg-gray-50 rounded-lg p-4">
        <div className="flex items-start space-x-2">
          ...
          <div className="text-xs text-[var(--color-primary)]/70">
            <p className="font-medium mb-1">{t('cashflowStmt.methodologyNoteLabel')}</p>
            {activeTab === 'indirect' ? (
              <p>{t('cashflowStmt.methodologyIndirect')}</p>
            ) : (
              <p>{t('cashflowStmt.methodologyDirect')}</p>
            )}
            <p className="mt-1">{t('cashflowStmt.complianceNote')}</p>
          </div>
        </div>
      </div>
    </div>
