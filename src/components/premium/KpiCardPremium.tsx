...
import { AtlasSparkline } from '../charts';
...
const KpiCardPremium: React.FC<KpiCardPremiumProps> = ({
  ...
  const c = TONE_COLORS[tone];
  const hasSpark = (series?.length ?? 0) > 1;
  ...
  return (
    ...
    >
      ...
      {meta && (
        <p
          className="text-xs"
          style={{ color: 'var(--color-text-tertiary)', marginBottom: hasSpark ? '0.625rem' : 0 }}
        >
          {meta}
        </p>
      )}
      ...
      {hasSpark && (
        <div className="-mx-1 -mb-1 mt-1">
          <AtlasSparkline data={series!} color={c.stroke} strokeWidth={1.75} height={48} title={eyebrow} />
        </div>
      )}
