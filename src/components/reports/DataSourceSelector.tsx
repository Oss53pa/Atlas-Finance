    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-primary-900">Sources de données</h3>
      ...
      {selectedSources.length > 0 && (
        <div className="p-3 bg-primary-50 rounded-lg border border-primary-100">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-medium text-primary-700">Sources sélectionnées :</span>
          </div>
          <div className="flex flex-wrap gap-2">
            {getSelectedSourcesInfo().map(source => (
              <div
                key={source.id}
                className="flex items-center gap-2 px-3 py-1.5 bg-white rounded-lg border border-primary-200"
              >
                {source.icon && <source.icon className="w-4 h-4 text-primary-600" />}
                <span className="text-sm font-medium text-primary-900">{source.name}</span>
                <button
      ...
      <div className="flex gap-3">
        ...
        <select
          ...
          className="px-3 py-2 border border-primary-200 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-primary-500"
        >
          {CATEGORIES.map(cat => (
      ...
      {/* Data sources list */}
      <div className="border border-primary-200 rounded-lg divide-y divide-primary-100 max-h-80 overflow-y-auto">
            ...
            <div key={source.id} className="bg-white">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="font-medium text-primary-900 truncate">{source.name}</p>
              ...
              {/* Expanded details */}
              {isExpanded && (
                <div className="px-12 pb-3 text-sm space-y-2 bg-gray-50 border-t border-primary-100">
                  <div className="grid grid-cols-2 gap-4 pt-2">
                    <div>
                      <span className="text-primary-500">Lignes :</span>
                      <span className="ml-2 font-medium text-primary-900">
                        {source.rowCount?.toLocaleString('fr-FR')}
                      </span>
                    </div>
                    <div>
                      <span className="text-primary-500">Catégorie :</span>
                      <span className="ml-2 font-medium text-primary-900">{source.category}</span>
                    </div>
                    <div>
                      <span className="text-primary-500">Dernière MAJ :</span>
                      <span className="ml-2 font-medium text-primary-900">
