...
/**
 * Atlas FnA Catalog — FULL independent section
 * Displays ALL tables, dashboards, charts, KPIs from Atlas FnA.
...
// createBlockFromCatalog : déplacé dans ../../data/createBlockFromCatalog
// (partagé avec le glisser-déposer du ReportBuilderApp).
...
const CatalogItemCard: React.FC<{
  ...
  // Le catalogue Atlas est la source de blocs de données du rapport : il doit
  // se glisser dans le canevas comme le catalogue de blocs de mise en page.
  // La poignée est l'icône (et non toute la carte) pour laisser les boutons
  // Ajouter / Aperçu / Favori cliquables.
  const { attributes, listeners, setNodeRef, isDragging } = useDraggable({
    id: `atlas-${item.id}`,
    data: { atlasItemId: item.id },
  });

  return (
    <div className={`group border border-neutral-200 rounded-lg p-3 hover:border-neutral-300 hover:shadow-sm transition-all bg-white ${isDragging ? 'opacity-50' : ''}`}>
      <div className="flex items-start gap-2.5">
        <div
          ...
          role="button"
          tabIndex={0}
          aria-label={`Glisser ${item.label} dans le rapport`}
          title="Glisser dans le rapport"
          className={`p-1.5 rounded-md border cursor-grab active:cursor-grabbing touch-none ${catColor}`}
        >
      ...
      {/* Actions — toujours visibles : « Ajouter au rapport » était le seul
          chemin fonctionnel et restait masqué jusqu'au survol (donc inatteignable
          au tactile), ce qui faisait passer le catalogue pour inerte. */}
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-3 pt-3 pb-2">
        <div className="text-xs font-bold text-neutral-800 mb-2">Catalogue Atlas FnA</div>
