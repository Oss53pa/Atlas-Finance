...
const SYSCOHADA_CATEGORIES = [
  // `nameKey` : le nom de classe est résolu au rendu (table figée au chargement).
  { code: '20', nameKey: 'assetClasses.class20', icon: FileText, color: 'red' },
  { code: '21', nameKey: 'assetClasses.class21', icon: FileText, color: 'red' },
  { code: '22', nameKey: 'assetClasses.class22', icon: Building, color: 'green' },
  { code: '23', nameKey: 'assetClasses.class23', icon: Building, color: 'blue' },
  { code: '24', nameKey: 'assetClasses.class24', icon: Package, color: 'orange' },
  { code: '25', nameKey: 'assetClasses.class25', icon: DollarSign, color: 'primary' },
  { code: '26', nameKey: 'assetClasses.class26', icon: TrendingUp, color: 'primary' },
  { code: '27', nameKey: 'assetClasses.class27', icon: TrendingUp, color: 'primary' },
];
...
const AssetsCategories: React.FC = () => {
  ...
  const handleSaveSettings = async () => {
    ...
    } catch (e: any) { toast.error(t('assetsCategories.saveFailed', { message: e?.message || t('assetsCategories.errorWord') })); }
  };
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[var(--color-border)]">
                    ...
                    <th className="text-center p-3 text-sm font-medium text-[var(--color-text-primary)]">{t('assetsCategories.colRate')}</th>
                ...
                <CardBody>
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      ...
                      <div>
                        <p className="text-xs text-[var(--color-text-secondary)]">{t('assetsCategories.colRate')}</p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-[var(--color-surface)] rounded-xl shadow-xl w-full max-w-3xl mx-4 max-h-[90vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 p-5 border-b border-[var(--color-border)]">
                ...
                <div><p className="text-xs text-[var(--color-text-secondary)]">{t('assetsCategories.accumDep')}</p><p className="font-bold text-red-600">{formatCurrency(c.amort)}</p></div>
                ...
                <div><p className="text-xs text-[var(--color-text-secondary)]">{t('assetsCategories.colRate')}</p><p className="font-bold text-[var(--color-text-primary)]">{c.depreciationRate}</p></div>
              </div>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-[var(--color-border)] text-[var(--color-text-secondary)]">
                          ...
                          <th className="text-right py-2 px-3 font-medium">{t('assetsCategories.accumDep')}</th>
                          <th className="text-right py-2 px-3 font-medium">{t('assetsCategories.nbv')}</th>
                          <th className="text-center py-2 pl-3 font-medium">{t('assetsCategories.colRateShort')}</th>
                        </tr>
                      </thead>
                      <tbody>
                      ...
                      <tfoot>
                        <tr className="border-t-2 border-[var(--color-border)] font-bold">
                          <td className="py-2 pr-3" colSpan={2}>{t('assetsCategories.totalPrefix', { code: c.code })}</td>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--color-text-primary)] mb-2">
                  {t('assetsCategories.classificationMethod')}
                </label>
                <select
                ...
                >
                  <option value="nature">{t('assetsCategories.byNature')}</option>
                ...
                <p className="text-xs text-[var(--color-text-secondary)] mt-1">
                  {t('assetsCategories.classificationHint')}
                </p>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 bg-[var(--color-background)] border border-[var(--color-border)] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                >
                  <option value="2">{t('assetsCategories.levels', { count: '2' })}</option>
                  <option value="3">{t('assetsCategories.levels', { count: '3' })}</option>
                  <option value="4">{t('assetsCategories.levels', { count: '4' })}</option>
                  <option value="unlimited">{t('assetsCategories.unlimited')}</option>
                </select>
                <p className="text-xs text-[var(--color-text-secondary)] mt-1">
                  {t('assetsCategories.hierarchyHint')}
                </p>
              </div>
