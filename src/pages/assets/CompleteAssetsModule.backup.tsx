  ...
  const wiseFMSync: WiseFMSync[] = [
    {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        />
        <StatCard
          title="Capteurs IoT actifs"
          value={stats.capteursIoT.toString()}
          icon={Wifi}
        ...
        <StatCard
          title="Alertes IA"
          value={stats.alertesIA.toString()}
          icon={Brain}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          title="Économies IA"
          value={`€${stats.economiesIA.toLocaleString('fr-FR')}`}
          icon={DollarSign}
        ...
        <StatCard
          ...
          value={stats.predictionsMaintenances.toString()}
          icon={Calendar}
        ...
        <StatCard
          title="Valeur nette"
          value={`€${stats.valeurNette.toLocaleString('fr-FR')}`}
          icon={TrendingDown}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <ModernCard>
          <CardHeader
            ...
            subtitle="Valeur nette vs Amortissement"
            icon={BarChart3}
          />
          <CardBody>
            <div className="h-64">
      ...
      <ModernCard>
        <CardHeader
          title="Inventaire physique en cours"
          icon={Camera}
          action={
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-sm font-medium text-blue-900">
                  Inventaire {inventory.type} - {new Date(inventory.date).toLocaleDateString('fr-FR')}
                </p>
                <p className="text-xs text-blue-700 mt-1">
                  Responsable: {inventory.responsable}
                </p>
              </div>
              {getStatutBadge(inventory.statut)}
            </div>
            <div className="grid grid-cols-3 gap-4 mb-4">
              <div>
                <p className="text-xs text-blue-600">Actifs à vérifier</p>
                <p className="text-xl font-bold text-blue-900">{inventory.nombreActifs}</p>
              </div>
              <div>
                <p className="text-xs text-blue-600">Vérifiés</p>
                <p className="text-xl font-bold text-blue-900">{inventory.nombreVerifies}</p>
              </div>
              <div>
                <p className="text-xs text-blue-600">Anomalies</p>
                <p className="text-xl font-bold text-red-600">{inventory.anomalies}</p>
              </div>
            </div>
            <div className="w-full bg-blue-100 rounded-full h-2">
              <div 
                className="bg-blue-600 h-2 rounded-full"
                style={{ width: `${(inventory.nombreVerifies / inventory.nombreActifs) * 100}%` }}
              />
            </div>
            <p className="text-xs text-blue-700 mt-2 text-center">
              Progression: {Math.round((inventory.nombreVerifies / inventory.nombreActifs) * 100)}%
            </p>
          </div>
        </CardBody>
      </ModernCard>
      ...
      <ModernCard>
        <CardHeader
          ...
          subtitle="Prochaines interventions"
          icon={Wrench}
        />
        <CardBody>
          <div className="space-y-3">
            {maintenances
              ...
              .map((maintenance) => {
                ...
                return (
                  <div key={maintenance.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                        maintenance.type === 'preventive' ? 'bg-blue-100 text-blue-600' :
                        maintenance.type === 'corrective' ? 'bg-red-100 text-red-600' :
                        'bg-gray-100 text-gray-600'
                      }`}>
                        <Wrench className="w-5 h-5" />
                      </div>
                      <div>
                        <p className="font-medium text-sm">{asset?.designation}</p>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        ...
                        <p className="text-sm font-medium">
                          {maintenance.prochaine ? new Date(maintenance.prochaine).toLocaleDateString('fr-FR') : 'À planifier'}
                        </p>
                      </div>
