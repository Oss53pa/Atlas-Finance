...
// Clés (et non libellés) : le tableau est figé au chargement du module, la
// traduction doit être résolue au rendu.
const LABEL_KEY: Record<CampagneStatut, string> = {
  preparation: 'budgetCampaign.stepPreparation', ouverte: 'budgetCampaign.stepOpen',
  consolidation: 'budgetCampaign.stepConsolidation', arbitrage: 'budgetCampaign.stepArbitration',
  votee: 'budgetCampaign.stepVoted', cloturee: 'budgetCampaign.stepClosed',
};
const NEXT_LABEL_KEY: Partial<Record<CampagneStatut, string>> = {
  preparation: 'budgetCampaign.nextOpen', ouverte: 'budgetCampaign.nextConsolidation',
  consolidation: 'budgetCampaign.nextArbitration', arbitrage: 'budgetCampaign.nextVote',
  votee: 'budgetCampaign.nextClose',
};
...
const BudgetCampagnePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) setError(e?.message || t('budgetCampaign.loadError'));
      } finally {
  ...
  const create = useCallback(async () => {
    ...
    try {
      ...
      if (!fy?.id) { setError(t('budgetCampaign.noActiveFiscalYear')); return; }
      await createCampagne(adapter, { fiscalYearId: fy.id, libelle: t('budgetCampaign.campaignName', { year: fy.code }) });
      setNotice(t('budgetCampaign.campaignCreated')); setRefreshKey((k) => k + 1);
    } catch (e: any) { setError(e?.message || t('budgetCampaign.failed')); } finally { setBusy(false); }
  }, [adapter, t]);
  ...
  const advance = useCallback(async () => {
    ...
    try {
      ...
      setNotice(t('budgetCampaign.statusChanged', { status: t(LABEL_KEY[next]) })); setRefreshKey((k) => k + 1);
    } catch (e: any) { setError(e?.message || t('budgetCampaign.transitionRefused')); } finally { setBusy(false); }
  }, [adapter, current, t]);
  ...
  const setTaux = useCallback(async (val: string) => {
    ...
    catch (e: any) { setError(e?.message || t('budgetCampaign.failed')); }
  }, [adapter, current, t]);
  ...
  return (
    <div className="p-6 space-y-5">
      <header className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-semibold text-[var(--color-text-primary)] flex items-center gap-2">
            <Megaphone className="w-6 h-6 text-[var(--color-primary)]" /> {t('budgetCampaign.title')}
          </h1>
          <p className="text-sm text-[var(--color-text-secondary)] dark:text-[var(--color-text-tertiary)]">{t('budgetCampaign.subtitle')}</p>
        </div>
        {!current && <button onClick={create} disabled={busy} className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-[var(--color-primary)] text-white text-sm font-medium hover:opacity-90 disabled:opacity-50"><Plus className="w-4 h-4" /> {t('budgetCampaign.newCampaign')}</button>}
      </header>
      ...
      {loading ? (
        <div className="flex items-center gap-2 text-[var(--color-text-secondary)] py-12 justify-center"><Loader2 className="w-5 h-5 animate-spin" /> {t('budgetCampaign.loading')}</div>
      ) : !current ? (
        <div className="rounded-2xl border border-[var(--color-border)] px-6 py-10 text-center text-sm text-[var(--color-text-secondary)]">
          {t('budgetCampaign.emptyState')}
        </div>
      ) : (
        <>
          <div className="bg-white rounded-xl border border-[var(--color-border)] shadow-sm p-5">
            <div className="flex items-center justify-between gap-3 flex-wrap mb-4">
              ...
              {NEXT_LABEL_KEY[current.statut] && (
                <button onClick={advance} disabled={busy}
                  ...
                  {busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <ArrowRight className="w-4 h-4" />} {t(NEXT_LABEL_KEY[current.statut]!)}
                </button>
              )}
            </div>
            {/* Stepper */}
            ...
            <div className="mt-4 flex items-center gap-3 flex-wrap text-sm">
              <label className="flex items-center gap-2 text-[var(--color-text-secondary)]">
                {t('budgetCampaign.defaultIndexationRate')}
          ...
          <div className="bg-white rounded-xl border border-[var(--color-border)] shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-[var(--color-border)] flex items-center justify-between">
              <span className="text-xs font-medium uppercase tracking-wide text-[var(--color-text-secondary)]">{t('budgetCampaign.completeness', { started: String(completude.started), total: String(completude.total), pct: String(completude.pct) })}</span>
            </div>
            {sections.length === 0 ? (
              <div className="py-8 text-center text-sm text-[var(--color-text-secondary)]">{t('budgetCampaign.noCostCentre')}</div>
            ) : (
                  ...
                  return (
                    <li key={s.id} className="px-4 py-2.5 flex items-center justify-between gap-3">
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-xs text-[var(--color-text-tertiary)]">{n > 0 ? t('budgetCampaign.lineCount', { count: String(n) }) : t('budgetCampaign.notStarted')}</span>
                        <button onClick={() => navigate(`/budget/saisie/${s.id}`)} className="text-xs text-[var(--color-primary)] dark:text-[var(--color-primary)] hover:underline">{t('budgetCampaign.enter')}</button>
                      </div>
                    </li>
                  );
                })}
