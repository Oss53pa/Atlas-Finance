...
const CapexEnveloppePage: React.FC = () => {
  ...
  const tiles = useMemo(() => state ? [
    { label: 'Votée', value: state.votee, accent: 'text-[var(--color-text-primary)]' },
  ...
  return (
    <div className="p-6 space-y-5">
      <header>
        <h1 className="text-2xl font-semibold text-[var(--color-text-primary)] flex items-center gap-2"><Wallet className="w-6 h-6 text-[var(--color-primary)]" /> Enveloppe CAPEX</h1>
        <p className="text-sm text-[var(--color-text-secondary)] dark:text-[var(--color-text-tertiary)]">Plan d'investissement annuel · exercice {fy?.code || '—'} · votée = réservée + appropriée + disponible + réserve</p>
      </header>
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            {tiles.map((t) => (
              <div key={t.label} className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-3">
                <div className="text-[11px] uppercase tracking-wide text-[var(--color-text-secondary)] leading-tight">{t.label}</div>
          ...
          <div className="bg-white rounded-xl border border-[var(--color-border)] shadow-sm p-5 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-2">
              ...
              <input type="number" value={vote} onChange={(e) => setVote(e.target.value)} placeholder="Montant voté" className="px-3 py-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] text-sm font-mono" />
              <input type="number" value={reserve} onChange={(e) => setReserve(e.target.value)} placeholder="Réserve %" className="px-3 py-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] text-sm font-mono" />
            ...
            <ul className="divide-y divide-[var(--color-border-light)]">
              {envs.length === 0 ? <li className="py-3 text-sm text-[var(--color-text-tertiary)] text-center">Aucune enveloppe votée.</li> : envs.map((e) => (
                <li key={e.id} className="py-2 flex items-center justify-between gap-2 text-sm">
                  <span className="text-[var(--color-text-secondary)]">{e.direction_section_id ? sections.find((s) => s.id === e.direction_section_id)?.libelle ?? 'Direction' : 'Société (global)'} <span className="text-xs text-[var(--color-text-tertiary)]">· réserve {Math.round(e.reserve_pct * 100)}%</span></span>
