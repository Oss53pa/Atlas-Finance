...
import {
  getActiveFiscalYear, createCapexRequest, dotationAnnuelle,
...
const CapexRequestModal: React.FC<Props> = ({ open, onClose, onCreated }) => {
  ...
  const reset = () => setF({ libelle: '', account_code: '', section_id: '', montant: '', date_prevue: '', duree_amortissement: '', methode: 'lineaire', valeur_residuelle: '', justification: '' });
  ...
  const submit = async () => {
    ...
    try {
      ...
      await createCapexRequest(adapter, {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) { reset(); onClose(); } }} containerClassName="max-w-2xl">
      <DialogContent>
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-gray-900">Demande d'investissement (CAR)</h3>
        ...
        <div className="grid grid-cols-2 gap-3">
          {field('Intitulé du projet / bien *', <input value={f.libelle} onChange={e => setF(s => ({ ...s, libelle: e.target.value }))} placeholder="ex. Groupe électrogène 250 kVA" className={inputCls} />)}
          {field('Compte immobilisation (classe 2) *', <>
            <input list="capex-accounts" value={f.account_code} onChange={e => setF(s => ({ ...s, account_code: e.target.value }))} placeholder="ex. 2411" className={inputCls + ' font-mono'} />
          ...
          {field('Montant (FCFA) *', <input type="number" value={f.montant} onChange={e => setF(s => ({ ...s, montant: e.target.value }))} placeholder="0" className={inputCls} />)}
          {field("Date d'acquisition prévue", <input type="date" value={f.date_prevue} onChange={e => setF(s => ({ ...s, date_prevue: e.target.value }))} className={inputCls} />)}
          {field("Durée d'amortissement (ans)", <input type="number" value={f.duree_amortissement} onChange={e => setF(s => ({ ...s, duree_amortissement: e.target.value }))} placeholder="ex. 5" className={inputCls} />)}
          {field('Méthode', <select value={f.methode} onChange={e => setF(s => ({ ...s, methode: e.target.value as any }))} className={inputCls}><option value="lineaire">Linéaire</option><option value="degressif">Dégressif</option></select>)}
          {field('Valeur résiduelle', <input type="number" value={f.valeur_residuelle} onChange={e => setF(s => ({ ...s, valeur_residuelle: e.target.value }))} placeholder="0" className={inputCls} />)}
          {field('Section / Centre', <select value={f.section_id} onChange={e => setF(s => ({ ...s, section_id: e.target.value }))} className={inputCls}><option value="">— Aucune —</option>{sections.map(s => <option key={s.id} value={s.id}>{s.code} · {s.libelle}</option>)}</select>)}
        </div>
        <div className="mt-3">{field('Justification', <textarea value={f.justification} onChange={e => setF(s => ({ ...s, justification: e.target.value }))} rows={2} placeholder="Motivation de l'investissement, ROI attendu…" className={inputCls} />)}</div>
        ...
        <div className="flex justify-end gap-2 mt-5">
          ...
          <button onClick={submit} disabled={saving} className="px-4 py-2 bg-green-600 text-white rounded-lg text-sm font-medium hover:bg-green-700 disabled:opacity-50 flex items-center gap-2"><CheckCircle className="w-4 h-4" />{saving ? 'Envoi…' : 'Créer la demande'}</button>
        </div>
      </DialogContent>
    </Dialog>
  );
