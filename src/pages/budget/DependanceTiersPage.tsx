...
// Renvoie une clé de traduction : le badge est résolu au rendu.
function hhiLabel(idx: number): { labelKey: string; cls: string } {
  if (idx >= 2500) return { labelKey: 'partyDependence.hhiHigh', cls: 'bg-red-100 text-red-700' };
  if (idx >= 1500) return { labelKey: 'partyDependence.hhiMedium', cls: 'bg-amber-100 text-amber-800' };
  return { labelKey: 'partyDependence.hhiLow', cls: 'bg-green-100 text-green-700' };
}
const RISK_KEY: Record<string, string> = {
  eleve: 'partyDependence.riskHigh', moyen: 'partyDependence.riskMedium', faible: 'partyDependence.riskLow',
};
...
const DependanceTiersPage: React.FC = () => {
  ...
  const load = useCallback(async (s: Side, v: CumulView) => {
    ...
    } catch (e) { toast.error(t('partyDependence.loadFailed', { message: (e as Error).message })); }
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-gray-900 flex items-center gap-2"><Network className="w-5 h-5 text-[var(--color-primary)]" /> {t('partyDependence.title')}</h1>
            <p className="text-sm text-gray-600">{t('partyDependence.subtitle', { year: refYear })}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex rounded-lg border border-gray-300 overflow-hidden">
            {CUMUL_VIEWS.map(v => <button key={v.key} onClick={() => setView(v.key)} className={`px-3 py-1.5 text-sm ${view === v.key ? 'bg-[var(--color-primary)] text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>{t(v.labelKey)}</button>)}
          </div>
          <button onClick={() => void load(side, view)} className="px-3 py-2 rounded-lg border border-gray-300 hover:bg-gray-50 flex items-center gap-2 text-sm"><RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} /> {t('partyDependence.reload')}</button>
        </div>
      </div>

      {/* Bascule clients / fournisseurs */}
      <div className="flex gap-2">
        {([['clients', 'partyDependence.sideClients', Users], ['fournisseurs', 'partyDependence.sideSuppliers', Truck]] as const).map(([key, labelKey, Icon]) => (
      ...
      {alerte && (
        <div className="p-3 rounded-lg border border-red-300 bg-red-50 flex items-start gap-2 text-sm text-red-800">
          ...
          <span><b>{t('partyDependence.alertTitle')}</b> {t(side === 'clients' ? 'partyDependence.alertClient' : 'partyDependence.alertSupplier', { pct: String(res!.top1Pct), threshold: String(seuil) })}</span>
        </div>
      )}
      ...
      {res && (
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          <Kpi label={t('partyDependence.kpiTop1')} value={`${res.top1Pct}%`} warn={res.top1Pct > seuil} />
          <Kpi label={t('partyDependence.kpiTop5')} value={`${res.top5Pct}%`} />
          <Kpi label={t('partyDependence.kpiTop10')} value={`${res.top10Pct}%`} />
          <Kpi label={t('partyDependence.kpiHhi', { count: String(res.nb) })} value={String(res.hhiIndex)} badge={hhi ? { label: t(hhi.labelKey), cls: hhi.cls } : undefined} />
          <Kpi label={t(side === 'clients' ? 'partyDependence.kpiTotalRevenue' : 'partyDependence.kpiTotalPurchases')} value={formatCurrency(res.total)} />
        </div>
      )}
      ...
      <div className="border border-gray-200 rounded-lg overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-200"><tr>
            ...
            <th className="text-left px-3 py-2 font-medium text-gray-700">{t('partyDependence.colParty')}</th>
            <th className="text-right px-3 py-2 font-medium text-gray-700">{t(side === 'clients' ? 'partyDependence.colRevenue' : 'partyDependence.colPurchases')}</th>
            <th className="text-right px-3 py-2 font-medium text-gray-700">{t('partyDependence.colShare')}</th>
            <th className="text-right px-3 py-2 font-medium text-gray-700">{t('partyDependence.colCumulative')}</th>
            {side === 'clients' && <th className="text-right px-3 py-2 font-medium text-gray-700">{t('partyDependence.colOutstanding')}</th>}
            {side === 'clients' && <th className="text-center px-3 py-2 font-medium text-gray-700">{t('partyDependence.colRisk')}</th>}
          </tr></thead>
          <tbody>
            {loading && <tr><td colSpan={7} className="px-3 py-8 text-center text-gray-500">{t('partyDependence.loading')}</td></tr>}
            {!loading && (!res || res.items.length === 0) && <tr><td colSpan={7} className="px-3 py-8 text-center text-gray-500">{t('partyDependence.noData')}</td></tr>}
              ...
              return (
                <tr key={it.code} className={`border-b border-gray-100 hover:bg-gray-50 ${franchi80 ? 'border-b-2 border-b-[var(--color-primary)]/40' : ''}`}>
                  ...
                  {side === 'clients' && <td className="px-3 py-2 text-center">
                    {it.risque
                      ? <span className={`px-2 py-0.5 rounded-full text-[11px] font-medium ${it.risque === 'eleve' ? 'bg-red-100 text-red-700' : it.risque === 'moyen' ? 'bg-amber-100 text-amber-800' : 'bg-green-100 text-green-700'}`}>{t(RISK_KEY[it.risque] ?? it.risque)}</span>
      ...
      <p className="text-xs text-gray-500">{t('partyDependence.footnote')}</p>
    </div>
  );
};
