    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 py-6">
          ...
          <p className="mt-1 text-gray-600">Système intégral conforme SYSCOHADA selon cahier des charges</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
              ...
              {[
                ...
                <button
                  ...
                  className={`py-4 border-b-2 font-medium text-sm flex items-center space-x-2 whitespace-nowrap ${
                    onglet === tab.id ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'
                  }`}
                >
            ...
            {onglet === 'types' && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                  {[
                    { type: 'Mensuelle', desc: 'Suivi régulier opérations courantes', echeance: 'J+5', color: 'bg-blue-50 border-blue-200' },
                    { type: 'Trimestrielle', desc: 'Consolidation et reporting intermédiaire', echeance: 'J+15', color: 'bg-green-50 border-green-200' },
                    { type: 'Semestrielle', desc: 'Bilan intermédiaire', echeance: 'J+20', color: 'bg-yellow-50 border-yellow-200' },
                        ...
                        <tr key={index} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-center">
                            <span className={`px-2 py-1 rounded text-xs ${
                              op.impact === 'FORT' ? 'bg-red-100 text-red-800' :
                              op.impact === 'MOYEN' ? 'bg-yellow-100 text-yellow-800' :
                              'bg-green-100 text-green-800'
                            }`}>{op.impact}</span>
                          </td>
                          <td className="px-4 py-3 text-center">
                            <span className={`px-2 py-1 rounded text-xs ${
                              op.statut === 'TERMINEE' ? 'bg-green-100 text-green-800' :
                              op.statut === 'VALIDEE' ? 'bg-blue-100 text-blue-800' :
                  ...
                  {[
                    ...
                    <div key={index} className="flex-1">
                      <div className={`p-4 rounded-lg border text-center ${
                        ...
                        <div className="text-xs text-gray-600 mt-1">{etape.user}</div>
                        <div className="mt-2">
                          {etape.statut === 'TERMINE' ? (
                            <CheckCircle className="h-4 w-4 text-green-500 mx-auto" />
                          ) : etape.statut === 'EN_COURS' ? (
                            <div className="h-4 w-4 border-2 border-blue-500 rounded-full mx-auto animate-spin"></div>
                          ) : (
                  ...
                  <div>
                    ...
                    {[
                      ...
                      <div key={index} className="p-4 border rounded mb-3">
                        ...
                        <div className="text-sm text-gray-600">Taille: {archive.taille} • {archive.chiffrement}</div>
                        <span className="px-2 py-1 bg-green-100 text-green-800 rounded text-xs">{archive.statut}</span>
                      </div>
                    ))}
                  </div>
                  <div>
                    ...
                    {[
                      ...
                      <div key={index} className="p-3 border rounded mb-3">
                        ...
                        <div className="text-xs text-gray-600">{audit.user} • {audit.date}</div>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mt-6">
          <h3 className="font-medium text-blue-900 mb-3">Conformité SYSCOHADA Respectée</h3>
