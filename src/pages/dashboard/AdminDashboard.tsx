...
const AdminDashboard: React.FC = () => {
  ...
  const handleBackup = async () => {
    ...
    try {
      ...
      toast.error(t('adminDashboard.backupFailed'));
    }
  ...
  const handleCreateUser = () => {
    ...
    toast.info(t('adminDashboard.userCreationMoved'));
  ...
  const confirmDeleteUser = () => {
    ...
    toast.info(t('adminDashboard.userDeletionMoved'));
  ...
  const systemMetrics = [
    {
      title: t('adminDashboard.metricEntries'),
      value: liveMetrics ? formatNumber(liveMetrics.entries) : '...',
      change: t('adminDashboard.metricEntriesSub'),
      color: 'blue',
      icon: Database,
      status: 'normal'
    },
    {
      title: t('adminDashboard.metricAccounts'),
      value: liveMetrics ? formatNumber(liveMetrics.accounts) : '...',
      change: t('adminDashboard.metricAccountsSub'),
      color: 'green',
      icon: Activity,
      status: 'good'
    },
    {
      title: t('adminDashboard.metricParties'),
      value: liveMetrics ? formatNumber(liveMetrics.thirdParties) : '...',
      change: t('adminDashboard.metricPartiesSub'),
      color: 'orange',
      icon: Users,
      status: liveMetrics && liveMetrics.thirdParties === 0 ? 'warning' : 'good'
    },
    {
      title: t('adminDashboard.metricAssets'),
      value: liveMetrics ? formatNumber(liveMetrics.assets) : '...',
      change: t('adminDashboard.metricAssetsSub'),
      color: 'green',
      icon: HardDrive,
      status: 'good'
    }
  ...
  const securityLogs = liveAuditLogs.length > 0 ? liveAuditLogs : [
    { time: '--:--', event: t('adminDashboard.noEvent'), user: 'system', ip: 'local', type: 'info' },
  ];
        ...
        {activeTab === 'system' && (
          <>
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border border-[var(--color-border)]">
              <h2 className="text-lg font-semibold text-[var(--color-text-primary)] mb-4">{t('adminDashboard.dbVolume')}</h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  ...
                  { label: t('adminDashboard.volParties'), value: liveMetrics?.thirdParties },
        ...
        {activeTab === 'users' && (
          <div className="bg-white rounded-xl shadow-sm border border-[var(--color-border)]">
            <div className="p-6 border-b border-[var(--color-border)]">
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-semibold text-[var(--color-text-primary)]">{t('adminDashboard.userManagement')}</h2>
                <button
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-[var(--color-background-secondary)]">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-[var(--color-text-secondary)] uppercase tracking-wider">{t('adminDashboard.colUser')}</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-[var(--color-text-secondary)] uppercase tracking-wider">{t('adminDashboard.colRole')}</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-[var(--color-text-secondary)] uppercase tracking-wider">{t('adminDashboard.colStatus')}</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-[var(--color-text-secondary)] uppercase tracking-wider">{t('adminDashboard.colLastLogin')}</th>
                    <th className="px-6 py-3 text-center text-xs font-medium text-[var(--color-text-secondary)] uppercase tracking-wider">{t('adminDashboard.colActions')}</th>
                  </tr>
                </thead>
                    ...
                    <tr key={user.id} className="hover:bg-[var(--color-background-secondary)]">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${
                          ...
                          {t(user.status === 'active' ? 'adminDashboard.statusActive' : 'adminDashboard.statusInactive')}
                        </span>
                      </td>
        ...
        {activeTab === 'security' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white rounded-xl p-6 shadow-sm border border-[var(--color-border)]">
                <h3 className="text-lg font-semibold text-[var(--color-text-primary)] mb-4">{t('adminDashboard.securitySettings')}</h3>
                <p className="text-sm text-[var(--color-text-secondary)]">
                  {t('adminDashboard.securityNoticePre')}{' '}
                  <strong>{t('adminDashboard.securityNoticeLink')}</strong>
                  {t('adminDashboard.securityNoticePost')}
                </p>
              </div>
                  ...
                  >
                    {isBackingUp ? (
                      <>
                        ...
                        <span>{t('adminDashboard.backupInProgress')}</span>
                      </>
                    ) : (
            ...
            <div className="p-6 border-b border-[var(--color-border)]">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <h2 className="text-lg font-semibold text-[var(--color-text-primary)]">{t('adminDashboard.newUserModal')}</h2>
                </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                >
                  <option value="Comptable">{t('adminDashboard.roleAccountant')}</option>
            ...
            <div className="p-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between py-2 border-b border-gray-100">
                  <span className="text-gray-600">{t('adminDashboard.role')}</span>
                  <span className="font-medium">{selectedUser.role}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-gray-100">
                  <span className="text-gray-600">{t('adminDashboard.status')}</span>
                  <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                    selectedUser.status === 'active'
                      ? 'bg-[var(--color-success-lighter)] text-[var(--color-success-darker)]'
                      : 'bg-gray-100 text-gray-600'
                  }`}>
                    {t(selectedUser.status === 'active' ? 'adminDashboard.statusActive' : 'adminDashboard.statusInactive')}
                  </span>
                </div>
                <div className="flex justify-between py-2">
                  <span className="text-gray-600">{t('adminDashboard.lastLogin')}</span>
      ...
      {showEditUserModal && selectedUser && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('adminDashboard.fullNameLabel')}</label>
                <input
                  type="text"
            ...
            <div className="p-6 border-t border-[var(--color-border)] flex justify-end space-x-3">
              ...
              <button
                onClick={() => {
                  ...
                  toast.info(t('adminDashboard.userEditMoved'));
      ...
      {showDeleteUserModal && selectedUser && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full">
            <div className="p-6 border-b border-[var(--color-border)]">
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-semibold text-[var(--color-error)]">{t('adminDashboard.deleteUser')}</h2>
            ...
            <div className="p-6">
              ...
              <p className="text-center text-gray-700 mb-4">
                {t('adminDashboard.confirmDelete')} <strong>{selectedUser.name}</strong> ?
              </p>
