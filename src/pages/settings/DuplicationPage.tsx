...
const DUP_ERROR_KEY: Record<string, string> = {
  source_target_required: 'duplication.errSourceTargetRequired',
  same_company: 'duplication.errSameCompany',
  no_block: 'duplication.errNoBlock',
};
...
const DuplicationPage: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    catch (e) { toast.error(t('duplication.loadFailed', { message: (e as Error).message })); }
  ...
  const run = async () => {
    ...
    if (err) { toast.error(t(DUP_ERROR_KEY[err])); return; }
    const tgt = companies.find(c => c.id === target);
    if (!window.confirm(t('duplication.confirmCopy', { name: tgt?.nom ?? '' }))) return;
    setBusy(true); setReport(null);
    try { const r = await duplicateConfig(adapter, source, target, blocks); setReport(r); toast.success(t('duplication.done')); }
    catch (e) { toast.error(t('duplication.failed', { message: (e as Error).message })); }
  ...
  return (
    <div className="p-6 space-y-4 max-w-3xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-semibold text-gray-900 flex items-center gap-2"><Copy className="w-5 h-5 text-[var(--color-primary)]" /> {t('duplication.title')}</h1>
          <p className="text-sm text-gray-600">{t('duplication.subtitle')}</p>
        </div>
      </div>

      {!loading && !canDuplicate && (
        <div className="p-3 rounded-lg border border-amber-300 bg-amber-50 text-sm text-amber-800">
          {t('duplication.needTwoCompanies')}
        </div>
      )}
      ...
      <div className="border border-gray-200 rounded-lg p-4 bg-white grid grid-cols-1 sm:grid-cols-[1fr_auto_1fr] gap-3 items-end">
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1">{t('duplication.sourceCompany')}</label>
        ...
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1">{t('duplication.targetCompany')}</label>
      ...
      <div className="border border-gray-200 rounded-lg p-4 bg-white">
        <div className="text-sm font-medium text-gray-800 mb-2">{t('duplication.blocksToCopy')}</div>
        <div className="space-y-2">
          {DUP_BLOCKS.map(b => (
            <label key={b.key} className="flex items-center gap-2 text-sm text-gray-700">
              <input type="checkbox" checked={blocks.includes(b.key)} onChange={() => toggleBlock(b.key)} />{t(b.labelKey)}
            </label>
          ))}
        </div>
        <div className="mt-3 text-xs text-gray-500 flex items-start gap-1.5">
          ...
          {t('duplication.exclusionsNotice')}
        </div>
      </div>
      ...
      <button onClick={run} disabled={!canDuplicate || busy} className="px-4 py-2 bg-[var(--color-primary)] text-white rounded-lg text-sm font-medium flex items-center gap-2 disabled:opacity-50">
        <Copy className="w-4 h-4" />{t(busy ? 'duplication.duplicating' : 'duplication.duplicate')}
      </button>

      {/* Rapport de copie */}
      {report && (
        <div className="border border-green-200 rounded-lg overflow-hidden">
          <div className="px-3 py-2 bg-green-50 border-b border-green-200 text-sm font-semibold text-green-800 flex items-center gap-2"><CheckCircle className="w-4 h-4" />{t('duplication.copyReport')}</div>
          <table className="w-full text-sm">
            <tbody>
              {DUP_BLOCKS.filter(b => b.key !== 'analytique' && b.key in report).map(b => (
                <tr key={b.key} className="border-b border-gray-100">
                  <td className="px-3 py-2 text-gray-700">{t(b.labelKey)}</td>
                  <td className="px-3 py-2 text-right font-medium text-gray-900">{t('duplication.copiedCount', { count: String(report[b.key]) })}</td>
                </tr>
              ))}
              {report.analytique && (
                <tr className="border-b border-gray-100">
                  <td className="px-3 py-2 text-gray-700">{t('duplication.analyticsReferential')}</td>
                  <td className="px-3 py-2 text-right font-medium text-gray-900">
                    {report.analytique.skipped
                      ? <span className="text-amber-600 text-xs">{t('duplication.skipped', { reason: String(report.analytique.skipped) })}</span>
                      : t('duplication.analyticsCounts', { plans: String(report.analytique.plans ?? 0), axes: String(report.analytique.axes ?? 0), sections: String(report.analytique.sections ?? 0), rules: String(report.analytique.regles ?? 0) })}
                  </td>
                </tr>
              )}
              {Object.keys(report).length === 0 && <tr><td className="px-3 py-3 text-gray-400 text-sm">{t('duplication.noBlockCopied')}</td></tr>}
            </tbody>
          </table>
        </div>
      )}
