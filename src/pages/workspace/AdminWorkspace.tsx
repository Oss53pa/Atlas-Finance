    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-lg p-6 border-2 border-[#ef4444]">
        ...
        <div className="grid grid-cols-4 gap-3">
          {[{label:'Utilisateurs',icon:Users,path:'/security/users',color:'#ef4444'},{label:'Securite',icon:Lock,path:'/security',color:'#171717'},{label:'Systeme',icon:Server,path:'/admin/system',color:'#737373'},{label:'Logs',icon:FileText,path:'/admin/logs',color:'#ef4444'}].map((a,i) => (
            <button key={i} onClick={() => navigate(a.path)} className="p-4 rounded-lg border-2 border-dashed border-[#ef4444]/50 hover:border-[#ef4444]">
    ...
    <div className="min-h-screen bg-[#e5e5e5]">
      <header className="bg-white border-b sticky top-0 z-50">
        <div className="flex items-center justify-between px-6 py-3">
          ...
          <div className="flex items-center space-x-3">
            <button onClick={() => navigate('/executive')} className="px-6 py-2 bg-gradient-to-r from-[#ef4444] to-[#ef4444] rounded-lg text-white font-bold flex items-center space-x-2"><Home className="w-5 h-5" /><span>ATLAS FINANCE</span><ExternalLink className="w-4 h-4" /></button>
      ...
      <div className="flex">
        <aside className={`${sidebarOpen ? 'w-64' : 'w-0 overflow-hidden'} lg:w-64 bg-white border-r min-h-[calc(100vh-73px)] transition-all`}>
          <div className="p-4">
            <button onClick={() => navigate('/executive')} className="w-full p-4 bg-gradient-to-r from-[#ef4444] to-[#ef4444] rounded-lg text-white mb-6"><div className="flex items-center justify-center space-x-2 mb-2"><Home className="w-5 h-5" /><ExternalLink className="w-4 h-4" /></div><div className="text-sm font-semibold">Atlas Finance Complet</div></button>
            ...
            <div className="border-b mb-4 pb-4"><div className="text-xs font-semibold text-gray-500 uppercase mb-3">Mon espace</div>
              <div className="space-y-1">
                {[
                  ...
                  <button key={item.id} onClick={() => setActiveSection(item.id as typeof activeSection)} className={`${activeSection===item.id?'bg-[#ef4444]/10 text-[#ef4444]':'text-gray-600 hover:bg-gray-50'} w-full flex items-center justify-between px-3 py-2 rounded-lg`}>
            ...
            <div className="space-y-1">
              {atlasFinanceLinks.map(item => (
                <button key={item.id} onClick={() => navigate(item.path)} className="w-full flex items-center justify-between px-3 py-2 rounded-lg text-gray-600 hover:text-[#ef4444] hover:bg-gray-50">
