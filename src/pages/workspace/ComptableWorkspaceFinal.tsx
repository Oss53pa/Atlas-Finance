...
const ComptableWorkspaceFinal: React.FC = () => {
  ...
  const userData = { name: user?.name || 'Marie Dupont', email: user?.email || 'marie@test.com', role: user?.role || 'Comptable', phone: '+225 07 00 00 00', department: 'Comptabilite' };
  ...
  const renderProfile = () => (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="bg-white rounded-xl p-6 border">
          <h4 className="font-semibold mb-4 flex items-center"><Mail className="w-5 h-5 mr-2 text-[#6A8A82]" />Contact</h4>
        ...
        <div className="bg-white rounded-xl p-6 border">
          <h4 className="font-semibold mb-4 flex items-center"><Shield className="w-5 h-5 mr-2 text-[#6A8A82]" />Securite</h4>
          <button className="w-full p-3 border rounded-lg text-sm hover:border-[#6A8A82] mb-2">Changer mot de passe</button>
          <button className="w-full p-3 border rounded-lg text-sm hover:border-[#6A8A82] flex justify-between"><span>2FA</span><span className="text-xs bg-yellow-100 text-yellow-700 px-2 py-1 rounded">Off</span></button>
        </div>
      </div>
    </div>
  );
        ...
        <div className="space-y-3">
          {['Email', 'Push', 'Alertes'].map((n, i) => (
            <div key={i} className="flex justify-between p-3 border rounded-lg"><span>{n}</span>
              ...
              <div className="w-11 h-6 bg-gray-200 peer-checked:bg-[#6A8A82] rounded-full after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-full"></div></label>
            </div>
          ))}
        </div>
      </div>
  ...
  const renderHelp = () => (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[{icon: BookMarked, title: 'Documentation', color: '#6A8A82'}, {icon: Video, title: 'Videos', color: '#B87333'}, {icon: FileQuestion, title: 'FAQ', color: '#7A99AC'}].map((c, i) => (
          <button key={i} className="bg-white rounded-xl p-6 border hover:border-[#6A8A82] text-left">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white rounded-xl p-6 border"><h4 className="font-semibold mb-4 flex items-center"><MessageCircle className="w-5 h-5 mr-2 text-[#6A8A82]" />Chat Support</h4><button className="w-full py-3 bg-[#6A8A82] text-white rounded-lg">Demarrer</button></div>
        <div className="bg-white rounded-xl p-6 border"><h4 className="font-semibold mb-4 flex items-center"><Headphones className="w-5 h-5 mr-2 text-[#6A8A82]" />Telephone</h4><p className="text-lg font-bold">+225 27 00 00 00</p></div>
      </div>
    </div>
  );
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-lg p-6 border">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-semibold flex items-center"><ListTodo className="w-5 h-5 mr-2 text-[#6A8A82]" />Mes Taches</h2>
          <button onClick={() => setActiveSection('tasks')} className="text-sm text-[#6A8A82] hover:underline">Voir tout</button>
        </div>
      ...
      <div className="bg-white rounded-lg p-6 border">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-semibold flex items-center"><MessageSquare className="w-5 h-5 mr-2 text-[#6A8A82]" />Messages recents</h2>
          <button onClick={() => setActiveSection('chat')} className="text-sm text-[#6A8A82] hover:underline">Voir tout</button>
        </div>
        <div className="space-y-2">
          {[{name:'Jean Martin', msg:'Peux-tu verifier le compte 401?', time:'10:30'},{name:'Sophie Kone', msg:'Rapport mensuel envoye', time:'09:15'}].map((m, i) => (
            <div key={i} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center space-x-3">
                <div className="w-8 h-8 rounded-full bg-[#6A8A82] flex items-center justify-center text-white text-xs">{m.name[0]}</div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="relative">
              <button onClick={() => setUserMenuOpen(!userMenuOpen)} className="flex items-center space-x-2 px-3 py-2 rounded-lg hover:bg-gray-100">
                <div className="w-8 h-8 rounded-full bg-gradient-to-r from-[#6A8A82] to-[#5A7A72] flex items-center justify-center"><User className="w-4 h-4 text-white" /></div>
              ...
              {userMenuOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border py-2 z-50">
                  <button onClick={() => { setActiveSection('profile'); setUserMenuOpen(false); }} className="w-full flex items-center space-x-3 px-4 py-3 hover:bg-gray-50"><User className="w-5 h-5 text-[#6A8A82]" /><span>Mon profil</span></button>
                  <button onClick={() => { setActiveSection('settings'); setUserMenuOpen(false); }} className="w-full flex items-center space-x-3 px-4 py-3 hover:bg-gray-50"><Settings className="w-5 h-5 text-[#6A8A82]" /><span>Parametres</span></button>
                  <button onClick={() => { setActiveSection('help'); setUserMenuOpen(false); }} className="w-full flex items-center space-x-3 px-4 py-3 hover:bg-gray-50"><HelpCircle className="w-5 h-5 text-[#6A8A82]" /><span>Aide</span></button>
            ...
            <div className="border-b mb-4 pb-4"><div className="text-xs font-semibold text-gray-500 uppercase mb-3">Mon espace</div>
              <div className="space-y-1">
                {[
                  ...
                  <button key={item.id} onClick={() => setActiveSection(item.id as typeof activeSection)} className={`${activeSection===item.id?'bg-[#6A8A82]/10 text-[#6A8A82]':'text-gray-600 hover:bg-gray-50'} w-full flex items-center justify-between px-3 py-2 rounded-lg`}>
