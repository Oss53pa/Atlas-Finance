...
export const StatCard: React.FC<StatCardProps> = ({
  ...
  if (loading) {
    return (
      <div className={`bg-white rounded-lg border border-[var(--color-border)] p-4 ${className}`}>
  ...
  return (
    <div
      className={`
        bg-white rounded-lg border border-[var(--color-border)] p-4
      ...
      <div className="flex items-center justify-between">
        ...
        {trend && (
          ...
          >
            ...
            {trend.label && <span className="text-[var(--color-text-tertiary)]">· {trend.label}</span>}
          </div>
        )}
      </div>
    </div>
