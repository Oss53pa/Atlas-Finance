...
/**
 * Money class for safe financial calculations.
 * Wraps Decimal.js to avoid floating-point errors in accounting.
 */
export class Money {
  ...
  divide(divisor: number | string | Decimal): Money {
    if (new Decimal(divisor).isZero()) throw new Error('Division par zéro');
